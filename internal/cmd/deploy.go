...
var deployCmd = &cobra.Command{
	...
	Short: "Deploy a WordPress site from a zip file, or create a new one",
...
func init() {
	...
	deployCmd.Flags().StringVarP(&zipPath, "zip", "z", "", "Path to the zip file (optional if creating a new site)")
	deployCmd.Flags().StringVarP(&domain, "domain", "d", "", "Domain for the new site")
	deployCmd.Flags().StringVarP(&url, "url", "u", "", "URL for the new site")
	deployCmd.Flags().StringVarP(&dbName, "dbname", "n", "", "Database name")
	deployCmd.Flags().Bool("dry-run", false, "Perform a dry run without making any changes")
	deployCmd.Flags().Bool("local", false, "Run locally without SSH")
	deployCmd.Flags().Bool("create-new-site", false, "If zip is invalid or not provided, create a new blank WordPress site")

	deployCmd.Flags().String("user", "", "SSH username (default: current user)")
